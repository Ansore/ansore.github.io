...
def compress_photo():
    '''调用压缩图片的函数
    '''
    src_dir, des_dir = "blog/photos/", "blog/min_photos/"
...
def handle_photo():
    ...
    auth = oss2.Auth('LTAIZIBOz1yooHPb', 'Wzt9iJylaG3UbLrDpsYLFTZgNfMCnV')
    bucket = oss2.Bucket(auth, endpoint, 'ansore-album')
    file_list = []
    objs=islice(oss2.ObjectIterator(bucket,prefix='blog/photos/20',delimiter=''),None)
    for obj in objs:
        file_list.append(obj.key.replace("blog/photos/",""))
    print(file_list)
    list_info = []
...
def aliyun_operation():
    ...
    auth = oss2.Auth('LTAIZIBOz1yooHPb', 'Wzt9iJylaG3UbLrDpsYLFTZgNfMCnV')
    bucket = oss2.Bucket(auth, endpoint, 'ansore-album')
    dir_list={ 'blog/min_photos/', 'blog/photos/'}
    for dir in dir_list:
